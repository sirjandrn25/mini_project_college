...
const increaseRadiusRange = (status)=>{
    if(status){
        radius+=radius;
        sendHelpRequest();
    }

    driverQueryModal.hide();
    
}

const driverConfirmationQuery = ()=>{
    driverQueryModal = new bootstrap.Modal(document.getElementById('driver_query'), {
                  keyboard: false
                    });
    driverQueryModal.show();
    const driver_query_container = document.getElementById('driver_query_container');
    driver_query_container.innerHTML = `<p>Other Mechanic Not available in ${radius} meter radius range..</p>
                                        <p>Do you want to again request in ${radius*2} meter radius range?</p>`;
}

const sendHelpRequest = ()=>{
    global_data.append('radius',radius);
     const params = {
        'method':'POST',
    ...
    fetch(url,params).then(resp=>resp.json()).then(json_data=>{
        ...
        if(json_data.status){
            ...
            document.getElementById('id_problem_desc').value = "";
            document.getElementById('id_vehicle_image').value = "";
...
ws.onmessage = event=>{
    ...
    if(data.accept === false){
        ...
        d_lat = data.m_lat;
        d_lng = data.m_lng;
        show_btn.removeAttribute('disabled');
        document.getElementById("map_container").innerHTML = `<button type="button" onClick="enlarge()" id="map_ctrl_btn">large size</button>`;
...
const check_running_list = ()=>{
    const url = `http://${window.location.host}/tracker/driver/check_running_list/`;
...
const showRouting = ()=>{
   ...
   // routingControl.route();
    ...
    const resize_map = ()=>{
        ...
        map_ctrl_btn.innerHTML = "large size";
    }
